    ...
    <View style={[styles.container, { backgroundColor: colors.card }]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View>
            <Text style={[styles.title, { color: colors.text }]}>{result.item}</Text>
        ...
        <View style={styles.confidence}>
          <Text style={[styles.confText, { color: colors.text + '99' }]}>{Math.round(result.confidence * 100)}% confidence</Text>
          <TouchableOpacity><Text style={styles.link}>Why this guess?</Text></TouchableOpacity>
        </View>

        <View style={styles.impact}>
          <Ionicons name="leaf-outline" size={20} color="#10b981" />
          <Text style={styles.impactText}>Avoids ~{fmtMassKg(result.co2eKg, units)} CO₂e (Est.)</Text>
        </View>

        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>Disposal instructions</Text>
          <Text style={[styles.body, { color: colors.text }]}>{result.instructions}</Text>
        </View>

        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>Preparation</Text>
          {result.prep.map((p, i) => <Text key={i} style={[styles.bullet, { color: colors.text + 'cc' }]}>• {p}</Text>)}
        </View>
        ...
        <View style={[styles.footer, { borderTopColor: colors.border }]}>
          <TouchableOpacity><Text style={[styles.link, { color: '#10b981' }]}>Report incorrect</Text></TouchableOpacity>
          <TouchableOpacity><Text style={[styles.link, { color: '#10b981' }]}>View in Guide</Text></TouchableOpacity>
        </View>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  footer: { flexDirection: 'row', justifyContent: 'space-around', paddingTop: 8, borderTopWidth: 1 },
});
